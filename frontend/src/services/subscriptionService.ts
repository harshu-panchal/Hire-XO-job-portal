...
export const subscriptionService = {
  ...
  /**
   * Initialize Razorpay subscription for interview verification tier
   */
  async initializeInterviewTierRazorpaySubscription(tierId: string): Promise<{ success: boolean; subscriptionId: string; razorpayKeyId: string }> {
    try {
      const response = await apiClient.post("/payments/interview-tier/subscribe", { tierId });
      return response.data;
    } catch (error) {
      throw new Error(getErrorMessage(error));
    }
