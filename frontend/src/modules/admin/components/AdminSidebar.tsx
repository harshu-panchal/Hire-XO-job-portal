...
export function AdminSidebar({ onClose }: { onClose?: () => void }) {
  ...
  return (
    <div className="flex flex-col h-full bg-white text-slate-900 w-full border-r border-slate-200 transition-all duration-300">
      <div className="p-6 flex items-center justify-between border-b border-slate-100">
        <div className="flex flex-col gap-2">
          <img src={logo} alt="HireXO" className="h-12 w-auto object-contain self-start" />
          <span className="text-xs text-primary font-medium ml-2">Admin Panel</span>
        </div>
        {onClose && (
          <button
            onClick={onClose}
      ...
      <nav className="flex-1 px-4 py-6 overflow-y-auto">
        ...
        >
          <div className="space-y-1">
            <p className="px-3 text-xs font-medium text-slate-400 mb-3">
              Main Menu
            </p>
            {menuItems.map((item) => (
