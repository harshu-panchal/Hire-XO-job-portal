                                ...
                                >
                                    ...
                                    <div className="max-h-[70vh] overflow-y-auto">
                                        {notifications.map((notif) => (
                                            ...
                                            >
                                                <div className={`p-2 rounded-lg h-fit ${notif.type === 'success' ? 'bg-green-100 dark:bg-green-500/20 text-green-600 dark:text-green-400' :
                                                ...
                                                <div className="flex-1 min-w-0">
                                                    <p className={`text-sm leading-tight mb-1 ${notif.unread ? "font-bold text-slate-900 dark:text-white" : "font-medium text-slate-700 dark:text-white/80"}`}>
                                                        {notif.title}
                                                    </p>
                                                    <p className="text-xs text-slate-500 dark:text-white/50 mb-1 line-clamp-2">{notif.description}</p>
                ...
                >
                    <div className="w-9 h-9 rounded-full bg-primary flex items-center justify-center text-white overflow-hidden group-hover:ring-2 group-hover:ring-primary/50 transition-all">
                        {user?.profile?.profilePhoto ? (
                            <img src={user.profile.profilePhoto} alt="Admin" className="w-full h-full object-cover" />
                        ) : (
                            <User className="w-5 h-5" />
                        )}
                    </div>
                    <div className="hidden lg:block text-left">
                        <p className="text-sm font-medium text-slate-900 dark:text-white group-hover:text-primary transition-colors">{user?.name || "Admin"}</p>
                        <p className="text-xs text-slate-500 dark:text-white/50">{user?.role || "Administrator"}</p>
                    </div>
                </Link>
            </div>
        </header>
