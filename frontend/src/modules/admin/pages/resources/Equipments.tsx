...
export default function Equipments() {
  ...
  const [equipmentTypesString, setEquipmentTypesString] = useState("");
  ...
  const fetchResources = async () => {
    setLoading(true);
    try {
      const response = await adminService.getResources("equipments", {
        search: debouncedSearch,
        page,
        limit
      });

      const targetType = activeTab === "provide" ? "rent-out-equipment" : "rent-equipment";

      // Map and filter by type (since we get all equipment from backend currently)
      const mapped = response.data
        ...
        .filter((item: any) => item.equipmentType === targetType);
      ...
      console.error(error);
      toast.error("Failed to load equipment records");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleOpenModal = (item?: EquipmentItem) => {
    if (item) {
      setEditingItem(item);
      setFormData(item);
      setEquipmentTypesString(item.equipmentTypes?.join(", ") || "");
      setShowModal(true);
    }
  };
  ...
  const handleDelete = async (id: string) => {
    if (window.confirm("Are you sure you want to delete this record?")) {
      try {
        await adminService.deleteResource("equipments", id);
  ...
  const handleSave = async () => {
    ...
    try {
      const payload: any = {
        ...
        equipmentTypes: equipmentTypesString.split(",").map(s => s.trim()).filter(s => s)
      };

      await adminService.updateResource("equipments", editingItem._id, payload);
    ...
    <motion.div variants={containerVariants} initial="hidden" animate="show" className="space-y-6">
      ...
      >
        <div>
          ...
          <p className="text-slate-500 mt-1">Manage equipment rentals</p>
        </div>
        ...
        {data.map((item) => (
          <div key={item._id} className="bg-white rounded-xl border border-slate-200 p-6 flex flex-col h-full">
            ...
            <div className="space-y-2 text-sm mb-4 flex-grow">
              {item.equipmentTypes && item.equipmentTypes.length > 0 ? (
                <div className="flex items-start gap-2 text-slate-600">
                  <Settings className="w-4 h-4 text-slate-400 mt-0.5" />
                  <span>{item.equipmentTypes.slice(0, 3).join(", ")}{item.equipmentTypes.length > 3 ? "..." : ""}</span>
                </div>
              ) : null}
          ...
          <>
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-lg bg-white rounded-xl border border-slate-200 shadow-xl z-50 p-6 max-h-[90vh] overflow-y-auto">
              <div className="flex items-center justify-between mb-6">
                <h3 className="text-lg font-semibold text-slate-900">Edit Equipment Record</h3>
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div><label className="block text-sm font-medium text-slate-700 mb-1.5">Type (Comma sep)</label><input type="text" value={equipmentTypesString} onChange={(e) => setEquipmentTypesString(e.target.value)} className="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary/50" /></div>
