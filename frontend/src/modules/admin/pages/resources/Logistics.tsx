...
interface LogisticsItem {
  ...
  email?: string;
  serviceArea?: string;
  logisticsType: "provide-logistics" | "need-logistics";
...
export default function Logistics() {
  ...
  const fetchResources = async () => {
    setLoading(true);
    try {
      const allResources = await adminService.getResources("logistics");
      const targetType = activeTab === "provide" ? "provide-logistics" : "need-logistics";

      const filtered = allResources.data.filter((item: any) => item.logisticsType === targetType);
      ...
      console.error(error);
      toast.error("Failed to load logistics records");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = async (id: string) => {
    if (window.confirm("Are you sure you want to delete this record?")) {
      try {
        await adminService.deleteResource("logistics", id);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await adminService.updateResource("logistics", editingItem._id, payload);
    ...
    <motion.div variants={containerVariants} initial="hidden" animate="show" className="space-y-6">
      ...
      >
        <div>
          ...
          <p className="text-slate-500 mt-1">Manage logistics services</p>
        </div>
          ...
          <>
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-lg bg-white rounded-xl border border-slate-200 shadow-xl z-50 p-6 max-h-[90vh] overflow-y-auto">
              <div className="flex items-center justify-between mb-6">
                <h3 className="text-lg font-semibold text-slate-900">Edit Logistics Record</h3>
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div><label className="block text-sm font-medium text-slate-700 mb-1.5">Service Area</label><input type="text" value={formData.serviceArea || ""} onChange={(e) => setFormData({ ...formData, serviceArea: e.target.value })} className="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary/50" /></div>
