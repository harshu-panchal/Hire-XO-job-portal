...
interface CSMItem {
  ...
  csmType: "offer-csm-services" | "hire-csm";
...
export default function CSM() {
  ...
  const [data, setData] = useState<CSMItem[]>([]);
  const [loading, setLoading] = useState(true);
  const [searchTerm, setSearchTerm] = useState("");
  const [showModal, setShowModal] = useState(false);
  const [editingItem, setEditingItem] = useState<CSMItem | null>(null);
  const [formData, setFormData] = useState<Partial<CSMItem>>({});

  const fetchResources = async () => {
    setLoading(true);
    try {
      const allResources = await adminService.getResources("csm");
      const targetType = activeTab === "offer" ? "offer-csm-services" : "hire-csm";

      const filtered = allResources.data.filter((item: any) => item.csmType === targetType);
      ...
      console.error(error);
      toast.error("Failed to load CSM records");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleOpenModal = (item?: CSMItem) => {
    if (item) {
      setEditingItem(item);
      setFormData(item);
      setShowModal(true);
  ...
  const handleDelete = async (id: string) => {
    if (window.confirm("Are you sure you want to delete this record?")) {
      try {
        await adminService.deleteResource("csm", id);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await adminService.updateResource("csm", editingItem._id, payload);
  ...
  return (
    <motion.div variants={containerVariants} initial="hidden" animate="show" className="space-y-6">
      ...
      >
        <div>
          <h1 className="text-2xl font-semibold text-slate-900">CSM Services</h1>
          <p className="text-slate-500 mt-1">Civil Site Management</p>
        </div>
          ...
          <>
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-lg bg-white rounded-xl border border-slate-200 shadow-xl z-50 p-6 max-h-[90vh] overflow-y-auto">
              <div className="flex items-center justify-between mb-6">
                <h3 className="text-lg font-semibold text-slate-900">Edit CSM Record</h3>
