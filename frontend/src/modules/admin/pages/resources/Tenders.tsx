...
export default function Tenders() {
  const [activeTab, setActiveTab] = useState<"provide" | "apply">("provide");
  ...
  const handleDelete = async (id: string) => {
    if (window.confirm("Are you sure you want to delete this tender?")) {
      try {
        await adminService.deleteResource("tenders", id);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await adminService.updateResource("tenders", editingItem._id, payload);
    ...
    <motion.div variants={containerVariants} initial="hidden" animate="show" className="space-y-6">
      ...
      >
        <div>
          ...
          <p className="text-slate-500 mt-1">Manage tender opportunities</p>
        </div>
        <div className="flex bg-white p-1 rounded-lg border border-slate-200">
          <button
            ...
            className={`px-4 py-2 rounded-md text-sm font-medium transition-all ${activeTab === "provide"
          ...
          <button
            ...
            className={`px-4 py-2 rounded-md text-sm font-medium transition-all ${activeTab === "apply"
              ? "bg-primary text-white shadow-sm"
              : "text-slate-600 hover:bg-slate-50"
              }`}
          >
      ...
      <motion.div variants={itemVariants} className="grid grid-cols-3 gap-4">
        <div className="bg-white rounded-xl border border-slate-200 p-5">
          ...
          <p className="text-sm text-slate-500 mt-1">Total Tenders</p>
        </div>
        <div className="bg-white rounded-xl border border-slate-200 p-5">
          <div className="flex items-center gap-3 mb-3">
