  ...
  return (
    <>
      <div
        ...
        className="block group relative bg-white rounded-[2rem] p-4 border border-slate-200 shadow-sm hover:shadow-md hover:scale-[0.98] hover:border-primary/20 transition-all cursor-pointer"
      >
        ...
        <div className="flex items-center justify-between mb-3">
          ...
          <button
            onClick={toggleSave}
            className={`size-9 rounded-xl flex items-center justify-center active:scale-90 transition-transform border border-slate-100 ${isSaved ? "bg-primary/10 border-primary" : "bg-slate-50"}`}
          >
            <Bookmark
              className={`size-4 ${isSaved ? "fill-primary text-primary" : "text-slate-300"}`}
            />
          </button>
        </div>
            ...
            {job.requirements && job.requirements.length > 0 && (
              <div className="flex flex-wrap gap-1.5">
                {job.requirements.slice(0, 2).map((req, i) => (
                  <span
                    key={i}
                    className="text-[8px] font-black uppercase tracking-wider px-2 py-0.5 rounded-md bg-slate-100 text-slate-500"
                  >
                    {req.split(" ").slice(0, 2).join(" ")}
                  </span>
                ))}
                {job.requirements.length > 2 && (
                  <span className="text-[8px] font-black uppercase tracking-wider px-2 py-0.5 rounded-md bg-slate-100 text-slate-400">
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full px-4 py-3 rounded-xl bg-slate-50 border border-slate-200 text-sm font-medium focus:outline-none focus:ring-2 focus:ring-primary/20 transition-all"
                />
              </div>
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <label
                    ...
                    className="flex items-center justify-center gap-3 w-full px-4 py-4 rounded-xl bg-slate-50 border-2 border-dashed border-slate-300 hover:border-primary/50 transition-all cursor-pointer"
                  >
              ...
              <div>
                ...
                <textarea
                  ...
                  className="w-full px-4 py-3 rounded-xl bg-slate-50 border border-slate-200 text-sm font-medium focus:outline-none focus:ring-2 focus:ring-primary/20 transition-all resize-none"
                />
              </div>
              ...
              <div className="flex gap-3 pt-4">
                <button
                  type="button"
                  onClick={() => setShowApplicationModal(false)}
                  className="flex-1 px-6 py-4 rounded-xl bg-slate-100 text-slate-700 font-black text-sm uppercase tracking-widest hover:bg-slate-200 active:scale-95 transition-all"
                >
                  Cancel
                </button>
                <button
      ...
      {showSuccessModal && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl p-8 max-w-md w-full border border-slate-200 shadow-2xl text-center">
            ...
            <p className="text-slate-600 mb-2">
              Your application for <span className="font-bold text-primary">{job.title}</span> at{" "}
              <span className="font-bold">{job.company}</span> has been successfully submitted.
            </p>
