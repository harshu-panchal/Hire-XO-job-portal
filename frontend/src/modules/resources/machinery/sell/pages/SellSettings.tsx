...
const SettingItem = ({ icon: Icon, label, description, action, onClick }: SettingItemProps & { onClick?: () => void }) => (
  ...
  >
    ...
    <div className="flex-1 space-y-0.5 min-w-0">
      <h3 className="text-xs font-black tracking-tight leading-none uppercase italic text-slate-700">
        {label}
      </h3>
      <p className="text-[9px] font-black uppercase tracking-widest text-slate-400 mt-1 truncate">
        {description}
      </p>
    </div>
    <div>
  ...
  return (
    <div className="py-6 space-y-8 select-none bg-slate-50 min-h-screen text-slate-900">
      ...
      <div className="space-y-4">
        <h2 className="px-2 text-[10px] font-black uppercase tracking-[0.4em] text-indigo-600 italic">
          Merchant Status
        </h2>
        <div className="space-y-3">
          <SettingItem
            ...
            action={
              ...
              >
                <div
                  className={`absolute top-1 size-4 bg-white rounded-full transition-all ${presence ? "left-7" : "left-1"}`}
                />
              </button>
            }
          />
          <SettingItem
            ...
            action={
              ...
              >
                <div
                  className={`absolute top-1 size-4 bg-white rounded-full transition-all ${analytics ? "left-7" : "left-1"}`}
                />
              </button>
            }
          />
      ...
      {/* Governance */}
      <div className="space-y-4">
        <h2 className="px-2 text-[10px] font-black uppercase tracking-[0.4em] text-slate-400 italic">
          Governance
        </h2>
        <div className="space-y-3">
          <SettingItem
