    ...
    <div className="py-6 space-y-8 select-none bg-slate-50 min-h-screen text-slate-900">
      ...
      <div className="flex items-center justify-between">
        ...
        <Link
          ...
          className="size-12 rounded-2xl bg-indigo-600 text-white flex items-center justify-center shadow-lg shadow-indigo-500/20 active:scale-90 transition-transform border border-indigo-500/10"
        >
      ...
      <div className="grid grid-cols-3 gap-3">
        {statCards.map((stat, index) => (
          ...
          >
            <div
              className={`size-10 rounded-2xl ${stat.bgColor} flex items-center justify-center relative z-10`}
            >
      ...
      <div className="space-y-4">
        <h2 className="text-[10px] font-black uppercase tracking-[0.4em] text-indigo-600 px-1 italic">
          Operations Center
        </h2>
        <div className="space-y-2">
          {alerts.map((alert) => (
            <div
              key={alert.id}
              className="flex items-center gap-4 p-4 bg-white border border-slate-200 rounded-2xl group active:scale-[0.98] transition-all shadow-sm"
            >
              <div
                className={`size-1.5 shrink-0 rounded-full ${alert.urgent ? "bg-rose-500 animate-pulse shadow-[0_0_8px_rgba(244,63,94,0.4)]" : "bg-indigo-400"}`}
              />
              <div className="flex-1 min-w-0">
                <p className="text-[11px] font-bold leading-tight line-clamp-1 text-slate-700">
                  {alert.message}
                </p>
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between px-1">
          ...
          <Link
            to="/machinery/sell/inventory"
            className="text-[9px] font-black text-indigo-600 uppercase tracking-widest flex items-center gap-1 active:scale-95 transition-transform"
          >
      ...
      <div className="bg-gradient-to-br from-indigo-600 to-indigo-900 rounded-[2.5rem] p-8 text-white relative overflow-hidden group shadow-2xl shadow-indigo-500/20">
        <div className="relative z-10 flex items-center justify-between">
          <div className="space-y-1">
            <h3 className="text-xl font-black tracking-tight italic uppercase italic">
              Market Demand
            </h3>
            <p className="text-[9px] font-black uppercase tracking-[0.3em] opacity-60">
              Forklifts are trending in Pune
            </p>
          </div>
        </div>
