...
const SettingItem = ({ icon: Icon, label, description, action }: SettingItemProps) => (
  <div className="flex items-center gap-4 p-4 rounded-3xl bg-white dark:bg-slate-900 border border-slate-100 dark:border-white/5 shadow-sm group">
    ...
    <div>
      {action || (
        <ChevronRight className="size-5 text-slate-300 group-hover:text-amber-600 transition-colors" />
      )}
    </div>
  </div>
);
  ...
  return (
    <div className="py-6 space-y-8 select-none">
      ...
      <div className="space-y-4">
        <h2 className="px-1 text-xs font-black uppercase tracking-[0.2em] text-amber-600">
          Preferences
        </h2>
        <div className="space-y-3">
          ...
          <SettingItem
            ...
            action={
              ...
              >
                <div
                  className={`absolute top-1 size-4 bg-white rounded-full transition-all ${highPriority ? "left-7" : "left-1"}`}
                />
              </button>
            }
          />
      ...
      <div className="pt-4 pb-8 space-y-6">
        ...
        <div className="text-center">
          <p className="text-[8px] font-black uppercase tracking-[0.4em] text-slate-300 dark:text-slate-700">
            Market Version 2.4.0 (Enterprise)
          </p>
        </div>
      </div>
    </div>
