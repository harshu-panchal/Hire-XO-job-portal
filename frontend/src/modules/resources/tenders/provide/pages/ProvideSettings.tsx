...
import { Bell, Shield, Palette, CreditCard, LifeBuoy, LogOut, ChevronRight, Moon, Sun, Key, Users, Clock, X, Copy, Check, Plus, Trash2 } from "lucide-react";
            ...
            <div className="space-y-4">
                ...
                <div className="space-y-3">
                    ...
                    <SettingItem
                        ...
                        action={
                            <div className={`px-3 py-1 rounded-full text-xs font-black ${isDarkMode ? "bg-indigo-600/10 text-indigo-600" : "bg-amber-500/10 text-amber-600"}`}>
                                {isDarkMode ? "ON" : "OFF"}
                            </div>
                        }
                    />
            ...
            {showAPIModal && (
                <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
                    <div className="bg-white dark:bg-slate-900 rounded-3xl p-6 max-w-2xl w-full border border-slate-200 dark:border-white/10 shadow-2xl max-h-[80vh] overflow-y-auto">
                        ...
                        <div className="space-y-3">
                            {apiKeys.map((key) => (
                                <div key={key.id} className="p-4 bg-slate-50 dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-white/10">
                    ...
                    <div className="bg-white dark:bg-slate-900 rounded-3xl p-6 max-w-2xl w-full border border-slate-200 dark:border-white/10 shadow-2xl">
                        ...
                        <div className="space-y-3">
                            {personnel.map((person) => (
                                <div key={person.id} className="p-4 bg-slate-50 dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-white/10">
                                    <div className="flex items-center justify-between">
                                        <div className="flex items-center gap-3">
                                            <div className="size-10 rounded-full bg-indigo-100 dark:bg-indigo-950/30 flex items-center justify-center">
                                                <span className="text-sm font-black text-indigo-600">{person.name.split(' ').map(n => n[0]).join('')}</span>
                                            </div>
                                            <div>
                        ...
                        <div className="space-y-4">
                            <div className="p-4 bg-slate-50 dark:bg-slate-800 rounded-2xl">
                                <div className="flex items-center justify-between mb-3">
                                    ...
                                    >
                                        <div className={`absolute top-1 size-4 bg-white rounded-full transition-all ${is2FAEnabled ? "left-7" : "left-1"}`} />
                                    </button>
                                </div>
                                {is2FAEnabled && (
                                    <div className="px-3 py-2 bg-emerald-50 dark:bg-emerald-950/20 rounded-lg border border-emerald-200 dark:border-emerald-900">
                                        <p className="text-xs font-bold text-emerald-700 dark:text-emerald-500">2FA is currently enabled</p>
                                    </div>
                                )}
                            </div>
                        ...
                        <div className="space-y-2">
                            {["Default", "Ocean Blue", "Forest Green", "Sunset Orange"].map((theme) => (
                                <button
                                    ...
                                    className={`w-full flex items-center justify-between p-3 rounded-xl transition-all ${selectedTheme === theme
                    ...
                    <div className="bg-white dark:bg-slate-900 rounded-3xl p-6 max-w-md w-full border border-slate-200 dark:border-white/10 shadow-2xl">
                        ...
                        <div className="space-y-4">
                            <div className="p-4 bg-gradient-to-br from-indigo-50 to-purple-50 dark:from-indigo-950/20 dark:to-purple-950/20 rounded-2xl border border-indigo-200 dark:border-indigo-900">
                                <p className="text-xs font-black uppercase tracking-widest text-indigo-600 mb-2">Current Plan</p>
                                <p className="text-2xl font-black mb-1">Enterprise</p>
                                <p className="text-sm text-slate-600 dark:text-slate-400">$299/month • Unlimited tenders</p>
                            </div>
                            <div className="p-4 bg-slate-50 dark:bg-slate-800 rounded-2xl">
                                <p className="text-xs font-black uppercase tracking-widest text-slate-500 mb-2">Next Billing Date</p>
                    ...
                    <div className="bg-white dark:bg-slate-900 rounded-3xl p-6 max-w-md w-full border border-slate-200 dark:border-white/10 shadow-2xl">
                        ...
                        <div className="space-y-3">
                            <div className="p-4 bg-blue-50 dark:bg-blue-950/20 rounded-2xl border border-blue-200 dark:border-blue-900">
                                ...
                                <p className="text-xs text-slate-600 dark:text-slate-400">Complete guides and API references</p>
                            </div>
