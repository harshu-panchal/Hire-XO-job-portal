    ...
    useEffect(() => {
        if (existingTender) {
            setFormData({
                ...
                deadline: existingTender.deadline ? new Date(existingTender.deadline).toISOString().split('T')[0] : "",
                releaseDate: existingTender.postedAt ? new Date(existingTender.postedAt).toISOString().split('T')[0] : new Date().toISOString().split('T')[0],
                description: existingTender.description || "",
                requirements: existingTender.requirements && existingTender.requirements.length > 0 ? existingTender.requirements : [""]
            });
    ...
    return (
        <div className="py-6 space-y-8 select-none mb-24">
            ...
            <div className="space-y-1">
                <h1 className="text-2xl font-black tracking-tight">{isEditMode ? "Update Tender" : "Post New Tender"}</h1>
                    ...
                    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
                        ...
                        <div className="space-y-4">
                            <div className="flex items-center justify-between ml-1">
                                ...
                                <button onClick={addRequirement} className="flex items-center gap-1 text-[10px] font-black uppercase tracking-widest text-indigo-600">
                            ...
                            <div className="space-y-3">
                                {formData.requirements.map((req, index) => (
                                    <div key={index} className="flex items-center gap-2">
                                        ...
                                        />
                                        <button onClick={() => removeRequirement(index)} className="size-11 rounded-xl bg-rose-50 dark:bg-rose-950/20 text-rose-600 flex items-center justify-center shrink-0 active:scale-90 transition-transform">
                    ...
                    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
                        ...
                        >
                            ...
                            <div className="space-y-1">
                                ...
                                <p className="text-[9px] font-black uppercase tracking-widest text-slate-400">PDF, Word, Excel, ZIP (Max 10MB each)</p>
                            </div>
                        ...
                        {(files.length > 0 || documentUrls.length > 0) && (
                            <div className="space-y-3">
                                <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 ml-1">Selected Documents</label>
                                {documentUrls.map((url, index) => (
                                    <div key={`url-${index}`} className="flex items-center gap-3 p-4 bg-white dark:bg-slate-900 border border-slate-100 dark:border-white/5 rounded-2xl">
                                        <FileText className="size-5 text-emerald-600" />
                                        <div className="flex-1 min-w-0">
                                            <p className="text-xs font-bold truncate">Existing Document {index + 1}</p>
                                            <p className="text-[8px] font-black uppercase tracking-widest text-slate-400">Attached</p>
                                        </div>
                                        <button onClick={() => removeExistingUrl(index)} className="size-8 rounded-lg bg-rose-50 dark:bg-rose-950/20 text-rose-600 flex items-center justify-center active:scale-90 transition-transform">
                                            <X className="size-4" />
                                        </button>
                                    </div>
                                ))}
                                {files.map((file, index) => (
                                    <div key={`file-${index}`} className="flex items-center gap-3 p-4 bg-white dark:bg-slate-900 border border-indigo-200/50 dark:border-indigo-900/20 rounded-2xl">
                                        <FileText className="size-5 text-indigo-600" />
                                        <div className="flex-1 min-w-0">
                                            <p className="text-xs font-bold truncate">{file.name}</p>
                                            <p className="text-[8px] font-black uppercase tracking-widest text-slate-400">{(file.size / (1024 * 1024)).toFixed(2)} MB</p>
                                        </div>
                                        <button onClick={() => removeFile(index)} className="size-8 rounded-lg bg-rose-50 dark:bg-rose-950/20 text-rose-600 flex items-center justify-center active:scale-90 transition-transform">
                                            <X className="size-4" />
                                        </button>
                                    </div>
                                ))}
            ...
            <div className="fixed bottom-0 left-0 right-0 p-6 flex justify-center z-40 pointer-events-none">
                <div className="w-full max-w-[430px] flex gap-3 pointer-events-auto">
                    ...
                    >
                        {loading ? (isEditMode ? "Updating..." : "Publishing...") : (step === 3 ? (isEditMode ? "Update Tender" : "Publish Tender") : "Next Step")} <ChevronRight className="size-4" />
                    </button>
                </div>
            </div>
        </div>
