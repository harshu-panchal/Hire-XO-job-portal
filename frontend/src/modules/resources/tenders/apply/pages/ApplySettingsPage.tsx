...
import {
    Bell, Shield, Eye, CreditCard, HelpCircle, LogOut, ChevronRight,
    Moon, Sun, Smartphone, Mail, ArrowLeft, Lock, Key, CheckCircle2,
    Laptop, Tablet, Phone, ExternalLink, MessageSquare, AlertCircle
} from "lucide-react";
...
const ApplySettingsPage = () => {
        ...
        <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
            ...
            <div className="p-5 rounded-[2rem] border border-slate-100 dark:border-white/5 bg-slate-50/50 dark:bg-white/5 flex items-center gap-4">
                ...
                <div className="flex-1">
                    ...
                    <p className="text-[8px] font-black uppercase tracking-widest text-slate-400">Not enabled</p>
                </div>
                <button
    ...
    const VisibilitySection = () => (
        <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
            <div className="p-6 bg-slate-900 dark:bg-white rounded-[2.5rem] text-white dark:text-slate-900">
                <p className="text-[10px] font-black uppercase tracking-[0.2em] opacity-60 mb-2">Display Settings</p>
                <h3 className="text-xl font-black tracking-tight">Profile Visibility</h3>
                <p className="text-xs mt-2 opacity-80 font-bold">Control how your organization profile appears to other users.</p>
            </div>

            <div className="space-y-3">
                {[
                    { label: "Show in Search", desc: "Allow vendors to find your tenders via search", enabled: true },
                    { label: "Public Profile", desc: "Anyone with your link can view company details", enabled: true },
                    { label: "Show Success Rate", desc: "Display your tender completion percentage", enabled: false }
                ].map((item, i) => (
                    <div key={i} className="flex items-center gap-4 p-5 rounded-[2rem] bg-white dark:bg-slate-900 border border-slate-100 dark:border-white/5">
                        <div className="flex-1">
                            <p className="text-sm font-black">{item.label}</p>
                            <p className="text-[9px] font-black uppercase tracking-widest text-slate-400">{item.desc}</p>
                        </div>
                        <button className={`w-10 h-5 rounded-full relative transition-colors ${item.enabled ? 'bg-emerald-500' : 'bg-slate-200'}`}>
                            <div className={`absolute top-0.5 size-4 bg-white rounded-full transition-all ${item.enabled ? 'left-5.5' : 'left-0.5'}`} />
                        </button>
                    </div>
                ))}
            </div>
            ...
            <div className="bg-amber-50 dark:bg-amber-950/20 border border-amber-100 dark:border-amber-900/30 p-5 rounded-[2rem] flex gap-4">
                ...
                <p className="text-[10px] font-black text-amber-700 dark:text-amber-400 uppercase tracking-widest leading-relaxed">
                    Setting your profile to private will prevent you from receiving direct bids from verified vendors.
                </p>
            </div>
        </div>
    );
        ...
        <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
            <div className="p-8 bg-gradient-to-br from-violet-600 to-indigo-700 rounded-[2.5rem] text-white shadow-xl shadow-indigo-500/20 relative overflow-hidden">
                ...
                <div className="relative space-y-4">
                    <p className="text-[10px] font-black uppercase tracking-[0.3em] opacity-80">Wallet Balance</p>
                    ...
                    <div className="flex gap-2">
                        ...
                        <button className="flex-1 bg-black/20 text-white backdrop-blur-md py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest active:scale-95 transition-all">History</button>
                    </div>
                </div>
            </div>

            <div className="space-y-4">
                <h4 className="text-[10px] font-black uppercase tracking-widest text-slate-400 ml-1">Current Plan</h4>
                <div className="p-6 rounded-[2.5rem] border border-slate-200 dark:border-white/10 bg-white dark:bg-slate-900 flex items-center justify-between">
                    <div>
                        ...
                        <p className="text-[10px] font-black uppercase tracking-widest text-slate-400">Renewal Date: 24 Oct 2026</p>
                    </div>
                    <div className="text-right">
                        <p className="text-lg font-black">₹4,999</p>
                        <p className="text-[8px] font-black uppercase tracking-widest text-slate-400">Per Month</p>
                    </div>
                </div>
            </div>

            <div className="space-y-3">
                <h4 className="text-[10px] font-black uppercase tracking-widest text-slate-400 ml-1">Included in your plan</h4>
                {[
    ...
    const DevicesSection = () => (
        <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
            <div className="space-y-4">
                {[
                    { icon: Laptop, name: "MacBook Pro M2", location: "Mumbai, India", status: "Active Now", current: true },
                    { icon: Smartphone, name: "iPhone 15 Pro", location: "Pune, India", status: "Last active 2 hrs ago", current: false },
                    { icon: Tablet, name: "iPad Air Gen 5", location: "Pune, India", status: "Last active 1 day ago", current: false }
                ].map((device, i) => (
                    <div key={i} className={`p-5 rounded-[2rem] bg-white dark:bg-slate-900 border ${device.current ? 'border-violet-200 dark:border-violet-900/40 bg-violet-50/20' : 'border-slate-100 dark:border-white/5'} flex items-center gap-4`}>
                        <div className={`size-12 rounded-2xl flex items-center justify-center ${device.current ? 'bg-violet-600 text-white' : 'bg-slate-100 dark:bg-slate-800 text-slate-400'}`}>
                            <device.icon className="size-6" />
                        </div>
                        <div className="flex-1 min-w-0">
                            <h4 className="text-sm font-black truncate">{device.name}</h4>
                            <p className="text-[9px] font-black uppercase tracking-widest text-slate-400">{device.location} • {device.status}</p>
                        </div>
                        {!device.current && (
                            <button className="text-[9px] font-black uppercase tracking-widest text-rose-600 hover:text-rose-700 transition-colors">Logout</button>
                        )}
                    </div>
                ))}
            </div>
    ...
    const SupportSection = () => (
        <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
            ...
            <div className="space-y-3">
                <h4 className="text-[10px] font-black uppercase tracking-widest text-slate-400 ml-1">Common Questions</h4>
                {[
                    ...
                    <div key={i} className="p-4 bg-white dark:bg-slate-900 border border-slate-100 dark:border-white/5 rounded-2xl flex items-center justify-between group cursor-pointer hover:border-violet-200 transition-all">
            ...
            <div className="p-8 bg-slate-900 text-white rounded-[2.5rem] space-y-4">
                <h3 className="text-xl font-black tracking-tight leading-tight">Can't find what you're looking for?</h3>
                <button className="bg-white text-slate-900 px-6 py-3 rounded-2xl font-black text-xs uppercase tracking-widest active:scale-95 transition-all">Submit a Ticket</button>
            </div>
        </div>
    );
        ...
        <div className="py-6 space-y-8 select-none">
            ...
            <div className="space-y-1">
                ...
                <p className="text-slate-500 dark:text-slate-400 font-black text-[10px] uppercase tracking-widest">
                    {activeSection ? `Configure your ${activeSection.toLowerCase()} preferences` : "Manage your preferences and security"}
                </p>
            </div>
