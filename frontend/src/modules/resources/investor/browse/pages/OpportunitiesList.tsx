    ...
    if (loading) {
        return (
            <div className="flex items-center justify-center min-h-[50vh]">
                <div className="animate-pulse text-sm font-black uppercase tracking-widest text-slate-400">Loading Opportunities...</div>
            </div>
        );
    }
        ...
        <div className="py-6 space-y-6 select-none">
                ...
                <div className="flex gap-2 overflow-x-auto no-scrollbar pb-2">
                    {sectors.map((sector) => (
                        <button
                            ...
                            className={`px-4 py-2 rounded-full text-[10px] font-black uppercase tracking-widest whitespace-nowrap transition-all ${selectedSector === sector.id
            ...
            <div className="space-y-4">
                {filteredOpportunities.length === 0 ? (
                    <div className="text-center py-20 bg-white/50 dark:bg-slate-900/30 rounded-[3rem] border-2 border-dashed border-slate-200 dark:border-white/5">
                        ...
                        <p className="text-xs text-slate-400 font-bold uppercase tracking-widest mt-1">Try adjusting your filters</p>
                    </div>
                ) : (
                            ...
                            <Link to={`/investor/browse/opportunities/${opp.id}`} className="block">
                                    ...
                                    <div className="flex-1 min-w-0">
                                        <div className="flex items-center justify-between mb-1">
                                            ...
                                            <div className="flex items-center gap-1.5 text-slate-400">
                                                ...
                                                <span className="text-[9px] font-black uppercase tracking-widest">{opp.location || "Remote"}</span>
                                            </div>
                                        </div>
                                        <h3 className="font-black text-xl tracking-tight mb-2 group-hover:text-violet-600 transition-colors">{opp.title}</h3>
                                ...
                                <div className="grid grid-cols-2 gap-4 mb-6">
                                    <div className="bg-emerald-50 dark:bg-emerald-950/20 rounded-2xl p-4 border border-emerald-100 dark:border-emerald-900/30">
                                        ...
                                        <p className="text-xl font-black text-emerald-700 dark:text-emerald-500">₹{opp.amount || opp.seekingAmount || "Negotiable"}</p>
                                    </div>
                                    <div className="bg-violet-50 dark:bg-violet-950/20 rounded-2xl p-4 border border-violet-100 dark:border-violet-900/30">
                                        ...
                                        <p className="text-xl font-black text-violet-700 dark:text-violet-500">{opp.equity || "To be discussed"}</p>
                                    </div>
                                </div>
            ...
            <div className="bg-slate-900 dark:bg-white p-8 rounded-[3rem] text-white dark:text-slate-900 relative overflow-hidden">
                <div className="relative z-10 space-y-2">
                    ...
                    <p className="text-xs font-bold opacity-60">Get notifications for new opportunities matching your preferred sectors.</p>
                </div>
