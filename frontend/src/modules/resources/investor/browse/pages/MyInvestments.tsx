        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div>
            ...
            <h2 className="text-4xl font-black tracking-tight">
              {interests.length} <span className="text-2xl opacity-60">Total</span>
            </h2>
          </div>
        ...
        {filteredInterests.length === 0 ? (
          <div className="text-center py-20 bg-white rounded-[3rem] border-2 border-dashed border-slate-200">
            ...
            <Link
              ...
              className="text-xs font-black text-violet-600 uppercase tracking-widest mt-2 inline-block hover:underline"
            >
          ...
          filteredInterests.map((interest) => {
            ...
            return (
              ...
              >
                <div className="flex items-start justify-between mb-4">
                  <div className="flex items-start gap-4">
                    ...
                    <div>
                      ...
                      <h3 className="font-black text-lg tracking-tight group-hover:text-violet-600 transition-colors">
                        {resource.title}
                      </h3>
                  ...
                  <div
                    className={`px-4 py-1.5 rounded-full bg-${statusColor}-500/10 border border-${statusColor}-500/20`}
                  >
                    <span
                      className={`text-[8px] font-black uppercase tracking-widest text-${statusColor}-600`}
                    >
                      {interest.status}
                    </span>
                  </div>
                ...
                <div
                  className={`absolute top-0 right-0 w-1 h-full bg-${statusColor}-500 opacity-50`}
                />
              </div>
            );
          })
      ...
      <div className="bg-white border border-slate-200 rounded-[3rem] p-8 text-center space-y-4">
        ...
        <div className="space-y-1">
          ...
          <p className="text-xs font-bold text-slate-500 uppercase tracking-widest">
            A diversified portfolio is a strong portfolio.
          </p>
        </div>
        <button
