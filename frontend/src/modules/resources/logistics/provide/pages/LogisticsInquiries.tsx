import { Search, Phone, Mail, Filter, CheckCircle2, Clock, X, MapPin, Package, Calendar, DollarSign } from "lucide-react";
...
const LogisticsInquiries = () => {
    ...
    const [inquiries, setInquiries] = useState<Inquiry[]>([
        ...
        },
        {
            ...
            message: "Require temperature-controlled transport for pharmaceutical products. Delhi to Mumbai route.",
            time: "1 day ago",
            status: "New",
            initial: "AP",
    ...
    const handleMarkAsClosed = (id: number) => {
        setInquiries(inquiries.map(inq =>
            inq.id === id ? { ...inq, status: "Closed" as const } : inq
        ));
    };
                ...
                <div className="flex gap-2 overflow-x-auto pb-2 no-scrollbar">
                    {(["All", "New", "Replied", "Closed"] as const).map((status) => (
                        <button
                            ...
                            className={`whitespace-nowrap px-5 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all active:scale-95 ${statusFilter === status
                                        ...
                                        <div className="flex gap-2">
                                            ...
                                            {inquiry.status === "New" && (
                                                ...
                                                >
                                                    ...
                                                    <span className="text-[9px] font-black uppercase tracking-widest">Mark Replied</span>
                                                </button>
                                            )}
                    ...
                    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 pointer-events-none">
                        ...
                        >
                            ...
                            <div className="sticky top-0 bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-white/10 p-6 flex items-start justify-between z-10">
                                <div className="flex items-start gap-4">
                                    <div className={`size-16 rounded-2xl bg-gradient-to-br ${selectedInquiry.color} flex items-center justify-center text-white text-2xl font-black shadow-lg shrink-0`}>
                                        {selectedInquiry.initial}
                                    </div>
                                    <div>
                                        <h2 className="text-2xl font-black tracking-tight mb-1">{selectedInquiry.name}</h2>
                                        <p className="text-sm font-bold text-slate-500 dark:text-slate-400">{selectedInquiry.role}</p>
                                        {selectedInquiry.company && (
                                            <p className="text-xs font-black uppercase tracking-widest text-orange-600 mt-1">{selectedInquiry.company}</p>
                                        )}
                                    </div>
                                </div>
                                <button
                            ...
                            <div className="p-6 space-y-6">
                                {/* Status & Time */}
                                <div className="flex items-center justify-between">
                                    <div className={`px-4 py-2 rounded-xl text-xs font-black uppercase tracking-widest ${selectedInquiry.status === "New"
                                            ? "bg-orange-600 text-white"
                                    ...
                                    <span className="text-xs font-bold text-slate-400 uppercase tracking-widest">{selectedInquiry.time}</span>
                                </div>

                                {/* Message */}
                                <div className="bg-slate-50 dark:bg-slate-800/50 rounded-2xl p-5">
                                    <h3 className="text-xs font-black uppercase tracking-widest text-slate-400 mb-2">Inquiry Message</h3>
                                    ...
                                    {selectedInquiry.cargo && (
                                        <div className="bg-white dark:bg-slate-800 rounded-2xl p-4 border border-slate-200 dark:border-white/10">
                                            <div className="flex items-center gap-2 mb-2">
                                                ...
                                                <span className="text-[9px] font-black uppercase tracking-widest text-slate-400">Cargo Type</span>
                                            </div>
                                    ...
                                    {selectedInquiry.date && (
                                        <div className="bg-white dark:bg-slate-800 rounded-2xl p-4 border border-slate-200 dark:border-white/10">
                                            <div className="flex items-center gap-2 mb-2">
                                                ...
                                                <span className="text-[9px] font-black uppercase tracking-widest text-slate-400">Required Date</span>
                                            </div>
                                ...
                                {selectedInquiry.budget && (
                                    <div className="bg-gradient-to-br from-orange-50 to-red-50 dark:from-orange-950/20 dark:to-red-950/20 rounded-2xl p-5 border border-orange-200 dark:border-orange-900/20">
                                        <div className="flex items-center gap-2 mb-2">
                                            ...
                                            <span className="text-xs font-black uppercase tracking-widest text-orange-600">Budget Range</span>
                                        </div>
