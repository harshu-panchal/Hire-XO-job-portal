...
export const BrowseNavbar = () => {
  ...
  return (
    <header className="sticky top-0 z-50 bg-white/80 dark:bg-background/80 backdrop-blur-xl border-b border-slate-200/50 dark:border-white/5 px-5 py-4 flex items-center justify-between transition-all duration-300 select-none">
      ...
      >
        <div className="size-10 bg-red-600 rounded-xl flex items-center justify-center shadow-lg shadow-red-600/20">
          <span className="text-white font-black text-xl tracking-tighter italic">H</span>
        </div>
        <span className="text-xl font-black tracking-tighter">
          Logistics <span className="text-red-600">Browse</span>
        </span>
      </Link>
      <div className="flex gap-2.5 relative">
        <button
          ...
          {unreadCount > 0 && (
            <span className="absolute top-3.5 right-3.5 size-2.5 bg-red-600 rounded-full border-2 border-white dark:border-slate-900 animate-pulse"></span>
          )}
        </button>
        ...
        <NotificationDropdown
          ...
          viewAllPath="/logistics/browse/notifications"
        />
      </div>
    </header>
  );
