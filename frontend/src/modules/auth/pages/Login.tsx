...
import { ArrowLeft, Mail, Lock, Eye, EyeOff, ChevronRight, Briefcase, Building2, Package, ShieldCheck } from "lucide-react";
...
const Login = () => {
    ...
    const getResourceRedirectPath = (u: any) => {
        ...
        const resourcePaths: Record<string, string> = {
            investor: profile?.investorType === "want-to-invest" ? "/investor/browse/dashboard" : "/investor/seek/dashboard",
            tenders: profile?.tenderType === "apply-for-tenders" ? "/tenders/apply/dashboard" : "/tenders/provide/dashboard",
            equipments: profile?.equipmentType === "rent-equipment" ? "/equipments/rent/dashboard" : "/equipments/provide/dashboard",
            machinery: profile?.machineryType === "need-machinery" ? "/machinery/buy/dashboard" : "/machinery/sell/dashboard",
            ...
            logistics: profile?.logisticsType === "need-logistics" ? "/logistics/browse/dashboard" : "/logistics/provide/dashboard",
            vehicles: profile?.vehicleType === "rent-vehicles" ? "/vehicles/browse/dashboard" : "/vehicles/provide/dashboard",
        };
    ...
    const [roleMismatch, setRoleMismatch] = useState<{ actual: string; target: string; user: any } | null>(null);
    ...
    if (roleMismatch) {
        return (
            <div className="min-h-screen relative overflow-hidden bg-slate-50 dark:bg-background flex flex-col justify-center p-6 text-slate-900 dark:text-white">
                <div className={`absolute top-0 left-0 w-full h-[400px] bg-gradient-to-b ${config.accent} opacity-5 rounded-b-[4rem] -z-10`} />
                ...
                <div className="w-full max-w-[400px] mx-auto space-y-6">
                    <Card className="border-0 bg-white/90 dark:bg-slate-900/90 backdrop-blur-xl shadow-2xl p-6 text-center animate-in fade-in zoom-in-95 duration-300">
                        ...
                        <p className="text-sm text-slate-500 dark:text-slate-400 mb-6 leading-relaxed">
                            You logged in with a <strong className="text-slate-900 dark:text-white">{roleMismatch.actual}</strong> account,
                            but you are on the <strong className="text-slate-900 dark:text-white">{roleMismatch.target}</strong> portal.
                        </p>
    ...
    return (
        <div className="min-h-screen relative overflow-hidden bg-slate-50 dark:bg-background flex flex-col justify-center p-6 text-slate-900 dark:text-white">
            ...
            <div className={`absolute top-0 left-0 w-full h-[400px] bg-gradient-to-b ${config.accent} opacity-5 rounded-b-[4rem] -z-10 transition-colors duration-500`} />
            ...
            <div className="w-full max-w-[400px] mx-auto space-y-6">
                ...
                <div className="text-center space-y-3">
                    <div className={`inline-flex items-center justify-center size-16 rounded-2xl ${config.iconBg} shadow-xl shadow-current/5 mb-2 mx-auto scale-in duration-500`}>
