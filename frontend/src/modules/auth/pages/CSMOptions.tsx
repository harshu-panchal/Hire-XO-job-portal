...
import { ArrowLeft, HardHat, UserCheck } from "lucide-react";
...
const CSMOptions = () => {
    ...
    useEffect(() => {
        if (isAuthenticated && user?.role === "resource" && user.profile.csmType) {
            if (user.profile.csmType === "offer-csm-services") {
                navigate("/csm/provide/dashboard");
            } else if (user.profile.csmType === "hire-csm") {
                navigate("/csm/browse/dashboard");
            }
        }
    ...
    const handleOptionClick = (option: typeof options[0]) => {
        if (isAuthenticated && user?.role === "resource") {
            ...
            navigate(`/signup/resource/csm?type=${option.id}`);
        }
    };
        ...
        <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-slate-100 dark:from-background dark:via-slate-950 dark:to-background flex items-center justify-center p-5">
            <div className="w-full max-w-[430px]">
                ...
                <div className="text-center mb-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
                    <div className="inline-flex items-center justify-center size-16 bg-gradient-to-br from-rose-500 to-pink-600 rounded-2xl shadow-xl shadow-rose-500/20 mb-4">
                        <HardHat className="size-8 text-white" />
                    </div>
                    <h1 className="text-3xl font-black tracking-tighter mb-2">CSM Options</h1>
                    <p className="text-slate-600 dark:text-slate-400">
                        Choose your CSM preference
                    </p>
                </div>
                ...
                <div className="mt-8 p-4 bg-rose-50 dark:bg-rose-950/20 border border-rose-200 dark:border-rose-900 rounded-xl">
                    <p className="text-sm text-rose-900 dark:text-indigo-100 text-center">
                        {isAuthenticated && user?.role === "resource" ? (
                            ...
                            <span><strong>Note:</strong> Select the option that best matches your CSM needs</span>
                        )}
                    </p>
                </div>
            </div>
