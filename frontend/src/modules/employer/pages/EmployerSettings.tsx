  ...
  const sections = [
    {
      ...
      items: [
        { label: t('employer.settings.sections.account.items.profile_information'), icon: User, color: "text-blue-500", bg: "bg-blue-500/10", routeKey: "Profile Information" },
        { label: t('employer.settings.sections.account.items.company_details'), icon: Building2, color: "text-primary", bg: "bg-primary/10", routeKey: "Company Details" },
        { label: t('employer.settings.sections.account.items.wallet_balance'), icon: Wallet, color: "text-green-500", bg: "bg-green-500/10", value: `₹${user?.profile?.walletBalance || 0}`, routeKey: "Wallet Balance" },
        { label: t('employer.settings.sections.account.items.subscription_plan'), icon: CreditCard, color: "text-amber-500", bg: "bg-amber-500/10", routeKey: "Subscription Plan" },
      ]
    },
    {
      title: t('employer.settings.sections.preferences.title'),
      items: [
        { label: t('employer.settings.sections.preferences.items.notifications'), icon: Bell, color: "text-purple-500", bg: "bg-purple-500/10", routeKey: "Notifications" },
        { label: t('employer.settings.sections.preferences.items.security_privacy'), icon: ShieldCheck, color: "text-green-500", bg: "bg-green-500/10", routeKey: "Security & Privacy" },
        { label: t('employer.settings.sections.preferences.items.language'), icon: Globe, color: "text-slate-500", bg: "bg-slate-500/10", value: language, isLanguage: true },
      ]
    }
  ];
  ...
  return (
    <div className="py-6 space-y-8 select-none pb-24 relative">
      <div className="space-y-1 px-1">
        <h1 className="text-3xl font-black tracking-tighter">{t('employer.settings.role_title')} <br /><span className="text-primary">{t('employer.settings.page_title')}</span></h1>
        <p className="text-slate-500 font-black text-[10px] uppercase tracking-widest">{t('employer.settings.subtitle')}</p>
      </div>
      ...
      <div className="bg-white dark:bg-slate-900/50 p-6 rounded-[2.5rem] border border-slate-200 dark:border-white/10 flex items-center gap-5">
        <div className="size-20 rounded-[2rem] bg-primary/10 flex items-center justify-center overflow-hidden border-4 border-white dark:border-slate-800 shadow-xl">
          <img
            src={user?.profile?.profilePhoto || user?.profilePhoto || `https://api.dicebear.com/7.x/avataaars/svg?seed=${user?.name || 'User'}`}
            alt={user?.name || "User"}
      ...
      >
        ...
        <span className="text-[10px] font-black uppercase tracking-widest">{t('employer.settings.logout')}</span>
      </button>
      ...
      {showLanguageModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-5 bg-slate-900/40 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="w-full max-w-sm bg-white dark:bg-slate-900 rounded-[2.5rem] shadow-2xl p-6 relative animate-in zoom-in-95 duration-200">
            ...
            <div className="mb-6">
              <h3 className="text-xl font-black tracking-tight mb-1">{t('employer.settings.modal.title')}</h3>
              <p className="text-xs font-black uppercase tracking-widest text-slate-400">{t('employer.settings.modal.subtitle')}</p>
            </div>
